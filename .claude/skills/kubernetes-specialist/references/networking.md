...
1. **Least Privilege**: Only open required ports and protocols
1. **Service Selection**: Use ClusterIP by default, LoadBalancer sparingly
1. **DNS Names**: Use service DNS names, avoid hardcoded IPs
1. **TLS Termination**: Terminate TLS at Ingress when possible
1. **Health Checks**: Configure proper health check paths
1. **Rate Limiting**: Apply rate limits at Ingress level
1. **Monitoring**: Expose metrics endpoints for Prometheus
